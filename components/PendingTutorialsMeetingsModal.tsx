            ...
            <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                {tutorials.length > 0 ? tutorials.map(tut => {
                    ...
                    return (
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    <h4 className="font-semibold text-green-800 text-lg mb-2">
                                        {tut.type === 'tutorial' ? 'Tutoría' : 'Reunión de Grupo'}
                                    </h4>
                                    <div className="space-y-1">
                                        {course && <p className="text-sm font-medium text-blue-700">{course.name}</p>}
                                        <p className="text-sm text-gray-700"><span className="font-semibold">Nombre del grupo:</span> {group?.name || 'Grupo desconocido'}</p>
                                        <p className="text-sm text-gray-700"><span className="font-semibold">Nombre del proyecto:</span> {project?.name || 'Sin proyecto asignado'}</p>
                                        <p className="text-sm text-gray-700"><span className="font-semibold">Tutor:</span> {findTutorName(tut.tutorId)}</p>
                                    </div>
                     ...
                     <p className="py-8 text-center text-gray-500">No hay elementos pendientes.</p>
                )}
            </div>
